...
set "SOURCE_PATH=E:\PiCz\SoNsTiGe"

REM Set path to Dr.Memory binary
set "DR_MEMORY=C:\Program Files (x86)\Dr. Memory\bin\drmemory.exe"
...
REM Disable VLD for now
for %%i in (%MPARALLEL32%) do (
	echo [Options]>  "%%~di%%~pi\vld.ini"
	echo VLD = off>> "%%~di%%~pi\vld.ini"
)
...
dir /B "%SOURCE_PATH%\*.jpg" | "%DR_MEMORY%" "%MPARALLEL32%" --stdin --no-split-lines --logfile="%~dp0\tmp\~logfile.txt" --pattern="%PATTERN%"
